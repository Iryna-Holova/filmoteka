export default class MakeMarkup {
  ...
  makeMovieCardMarkupNightTheme(movies) {
    let genres = [];
    let releaseDate = 'No date';
    if (movies) {
      const movieCardMarkupNightTheme = movies
        .map(({ title, poster_path, release_date, genre_ids, id }) => {
          if (genre_ids.length === 0) {
            genres = genre_ids.slice(0, 1);
            genres.splice(0, 0, 'No genre');
          ...
          if (poster_path) {
            return `<li class="film-list__item" data-id="${id}">
                    ...
                    <div class="film-list__description">
                        <h2 class="film-list__title night">${title}</h2>
                        <p class="film-list__genres">${Object.values(genres).join(
                          ', ',
                        )}<span> | </span>${releaseDate}</p>
                    </div>
      ...
      return movieCardMarkupNightTheme;
    }
  }

  makeWatchedMovieCardMarkupNightTheme(movies) {
    let releaseDate = 'No date';
    let movieGenres = [];
    let moviGenresNames = [];
    if (movies) {
      const libraryMovieCardMarkupNightTheme = movies
        .reverse()
        .map(({ title, poster_path, release_date, genres, vote_average, id }) => {
          if (release_date && release_date !== '') {
            releaseDate = release_date.slice(0, 4);
      ...
      return libraryMovieCardMarkupNightTheme;
    }
  } 
  
  makeQueueMovieCardMarkupNightTheme(movies) {
    let releaseDate = 'No date';
    let movieGenres = [];
    let moviGenresNames = [];
    if (movies) {
  ...
  makeMovieDetailMarkupNightTheme({
    poster_path,
    title,
    vote_average,
    vote_count,
    ...
    let movieDetailMarkupNightTheme;
    if (poster_path) {
      movieDetailMarkupNightTheme = `
        <div class="modal-gallery__thumb">
            <img src="https://image.tmdb.org/t/p/original${poster_path}" alt="${title} poster" onerror="this.onerror=null;this.src='https://bflix.biz/no-poster.png'" />
        </div>
        <div class="modal-gallery__thumb-text">
            <h2 class="modal-gallery__title night">${title}</h2>
            <ul class="modal-gallery-list">
                <li class="modal-gallery-list__item">
                    <div class="modal-gallery-item__name">Vote / Votes</div>
                    <div class="modal-gallery-item__value">
                ...
                <li class="modal-gallery-list__item">
                    <div class="modal-gallery-item__name">Popularity</div>
                    <div class="modal-gallery-list__popularity modal-gallery-item__value night">${popularity.toFixed(1)}</div>
                </li>
                <li class="modal-gallery-list__item">
                    <div class="modal-gallery-item__name">Original Title</div>
                    <div class="modal-gallery-list__title modal-gallery-item__value night">${original_title}</div>
                </li>
                <li class="modal-gallery-list__item">
                    <div class="modal-gallery-item__name">Genre</div>
                    <div class="modal-gallery-list__genre modal-gallery-item__value night">${genres
                        .map(genre => genre.name)
                        .join(', ')}</div>
                </li>
            </ul>
            <article class="modal-gallery-about">
                <h3 class="modal-gallery-about__title night">About</h3>
                <p class="modal-gallery-about__text night">${overview}</p>
            </article>
            <div class="modal-gallery-buttons__thumb">
                <button class="button modal-gallery-button" data-add="watched" data-id="${id}">Add to watched</button>
                <button class="button modal-gallery-button" data-add="queque" data-id="${id}">Add to queue</button>
